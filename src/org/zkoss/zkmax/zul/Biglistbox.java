/* BigListbox.java
	...
	History:
		Fri, Feb 17, 2012 11:14:36 AM, Created by jumperchen
 ...
 * the functionalities of {@link org.zkoss.zul.Listbox} including selection,
 ...
 * selection.</li>
 * <li>{@link ScrollEventExt} for onScroll is sent when user changes the scroll
 * position either x or y.</li>
 * <li>{@link ScrollEventExt} for onScrollY is sent when user changes the scroll
 * position y.</li>
 * <li>{@link ScrollEventExt} for onScrollX is sent when user changes the scroll
 * position x.</li>
 * <li>{@link SortEventExt} is sent when user sort the column.</li>
 * <li>{@link CellClickEvent} is sent when user click upon the cell field.</li>
 * <li>onAfterRender is sent when the model's data has been rendered.</li>
 * </ol>
 * 
 *  <h3>Custom Attributes</h3>
 *  
 * <dt>org.zkoss.zkmax.zul.biglistbox.preloadSize</dt>. 
 * <dd>Specifies the number of items to preload when receiving the rendering
 * request from the client.
 * 
 * <p>
 * Default {@link #getZclass}: z-biglistbox.
 * <p>
 * <b>Note:</b> This component can only support those browsers which support
 * HTML5/CSS3 selector. Like IE8+, Firefox, Safari, Opera, and Chrome.
 * 
 * @author jumperchen
 * @since 6.0.1
 * 
...
public class Biglistbox extends XulElement {
	...
	static {
		addClientEvent(Biglistbox.class, Events.ON_SELECT, CE_DUPLICATE_IGNORE
				| CE_IMPORTANT);
		addClientEvent(Biglistbox.class, Events.ON_SCROLL, CE_DUPLICATE_IGNORE);
		addClientEvent(Biglistbox.class, "onScrollY", CE_DUPLICATE_IGNORE
				| CE_IMPORTANT | CE_NON_DEFERRABLE);
		addClientEvent(Biglistbox.class, "onScrollX", CE_DUPLICATE_IGNORE
				| CE_IMPORTANT | CE_NON_DEFERRABLE);
		addClientEvent(Biglistbox.class, "onAdjustRows", CE_DUPLICATE_IGNORE
				| CE_IMPORTANT | CE_NON_DEFERRABLE);
		addClientEvent(Biglistbox.class, "onAdjustCols", CE_DUPLICATE_IGNORE
				| CE_IMPORTANT | CE_NON_DEFERRABLE);
	...
	private MatrixComparatorProvider<?> toComparator(String clsnm)
		...
		final Class cls = page != null ? page.resolveClass(clsnm) : Classes
	...
	public int getPreloadSize() {
		if (_preloadsz < 0) {
			...
			if ((sz = Utils.getIntAttribute(this,
					"org.zkoss.zkmax.zul.biglistbox.preloadSize", sz, true)) < 0)
	...
	private void syncModel0(int type) {
		if (_model != null) {
			...
			if ((type & (ALL|FROZEN_COLUMN)) != 0) {
				...
				smartUpdate("rowDataFx", new Object[] { 0, _currentY,
							_tmpRowDataFx});
			...
			if ((type & (ALL|COLUMN)) != 0) {
				...
				smartUpdate("colData", new Object[] { _currentX, _tmpColData });
			}
			
			if ((type & (ALL|ROW)) != 0) {
				_tmpRowData = syncRowData(initCols, endCols, initRows, endRows);
				smartUpdate("rowData", new Object[] { _currentX, _currentY,
						_tmpRowData });
			}
		...
		try {
			...
			for (int rowIndex = 0; rowIndex < headSize; rowIndex++) {
				...
				for (int col = 0, colIndex = initCols, clen = endCols; colIndex < clen; colIndex++) {
					tmpColData[rowIndex][col++] = renderer.renderHeader(
							this, value, rowIndex, colIndex);
				}
			}
		} catch (Exception e) {
			...
			for (int row = 0, rowIndex = initRows, rlen = endRows; rowIndex < rlen; row++, rowIndex++) {
				...
				for (int col = 0, colIndex = initCols, clen = endCols; colIndex < clen; colIndex++) {
					if (_jsel < 0 && smodel.isSelected(value))
						_jsel = rowIndex;
					tmpRowData[row][col++] = renderer.renderCell(this,
	...
	private static final MatrixRenderer<Object> _defMTRend = new MatrixRenderer<Object>() {
		@SuppressWarnings({ "unchecked", "rawtypes" })
		public String renderCell(final Component owner, final Object data,
				final int rowIndex, final int colIndex) {
			final Biglistbox self = (Biglistbox) owner;
			final Template tm = self.getTemplate("rows");
			if (tm == null)
				return Objects.toString(((MatrixModel) self.getModel())
						.getCellAt(data, colIndex));
			else {
				...
				throw new UiException(
						"The model template can only support Label or Html component, not "
								+ items[0]);
			}
		}

		@SuppressWarnings({ "unchecked", "rawtypes" })
		public String renderHeader(final Component owner, final Object data,
				final int rowIndex, final int colIndex) {
			final Biglistbox self = (Biglistbox) owner;
			final Template tm = self.getTemplate("heads");
			if (tm == null)
				return Objects.toString(((MatrixModel) self.getModel())
						.getHeaderAt(data, colIndex));
			else {
				final Component[] items = tm.create(owner, null,
						new VariableResolver() {
							public Object resolveVariable(String name) {
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public void setMatrixRenderer(String clsnm) throws ClassNotFoundException,
			NoSuchMethodException, IllegalAccessException,
			InstantiationException, java.lang.reflect.InvocationTargetException {
		if (clsnm != null)
			setMatrixRenderer((MatrixRenderer) Classes
			...
			_dataListener = new ListDataListener() {
				public void onChange(ListDataEvent event) {
					...
					case ListDataEvent.STRUCTURE_CHANGED:
						int colSize = _model.getColumnSize(),
						rowSize = _model.getSize();
	...
	protected void renderProperties(org.zkoss.zk.ui.sys.ContentRenderer renderer)
		...
		org.zkoss.zkex.rt.Runtime.init(this);
		int cacheSize = Utils.getIntAttribute(this,
				"org.zkoss.zkmax.zul.biglistbox.clientCacheSize", 1, true);
		if (cacheSize != 1) {
		...
		if (sm != null) {
			...
			if (_sortDsc != null)
				direction = sm.getSortDirection(_sortDsc
						.getColumnComparator(_sortingColumnIndex));
			if ("natural".equals(direction) && _sortAsc != null)
				direction = sm.getSortDirection(_sortAsc
						.getColumnComparator(_sortingColumnIndex));
		...
		if (_tmpRowDataFx != null) {
			render(renderer, "rowDataFx", new Object[] {
				0, _currentY, _tmpRowDataFx
			});
			_tmpRowDataFx = null;
		}
		...
		if (_tmpRowData != null) {
			render(renderer, "rowData", new Object[] { _currentX, _currentY,
					_tmpRowData });
	...
	public void service(org.zkoss.zk.au.AuRequest request, boolean everError) {
		...
		if (cmd.equals(Events.ON_SELECT)) {
			...
			final Integer x = ((Integer) request.getData().get("columnIndex"))
					.intValue();
			...
			if (prevSelected != null) {
				...
				Events.postEvent(new SelectEvent(Events.ON_SELECT, this, null,
						null, null, selObjs, prevSet, prevSet, null, new Integer[] { x, index }, 0));
			} else {
				Events.postEvent(new SelectEvent(Events.ON_SELECT, this, null,
						null, null, selObjs, null, null, null, new Integer[] { x, index }, 0));
			}
	...
	public boolean sort(boolean ascending, int columnIndex) {
		final String dir = getSortableModel().getSortDirection(
				ascending ? getSortAscending().getColumnComparator(columnIndex)
	...
	boolean doSort(boolean ascending, int columnIndex) {
		final MatrixComparatorProvider<?> cmpr = ascending ? _sortAsc
				: _sortDsc;
		if (cmpr == null)
			return false;
	...
	private synchronized void writeObject(java.io.ObjectOutputStream s)
		...
		s.writeObject(_matrixRenderer instanceof java.io.Serializable
				|| _matrixRenderer instanceof java.io.Externalizable ? _matrixRenderer
				: null);
	}
