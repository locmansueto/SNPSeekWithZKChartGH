...
public class VargroupListItemRenderer implements ListitemRenderer<List> {
	...
	public VargroupListItemRenderer(Map<String, String> mapGroup2Color) {
		super();
		this.mapGroup2Color = mapGroup2Color;
	}
	*/
	...
	public void render(Listitem listitem, List arg1, int row) throws Exception {
		...
		/*
		Iterator<String> itStr=arg1.iterator();
		while(itStr.hasNext()) {
		...
		*/
		...
		while(itStr.hasNext()) {
			...
			addListcell (listitem, str);
			/*
			String sti="color:black;align:center";
			if(str.equals("A")) sti=SNPRowRendererStyle.STYLE_A;
			else if(str.equals("T")) sti=SNPRowRendererStyle.STYLE_T;
			else if(str.equals("G")) sti=SNPRowRendererStyle.STYLE_G;
			else if(str.equals("C")) sti=SNPRowRendererStyle.STYLE_C;
			addListcell (listitem, str,sti);
			*/
			poscnt++;
		}
		...
		if(mapKgroup2AppendValues!=null) {
			...
			if(lValues==null) {
				for(int i=0; i<lHeader.size(); i++) addListcell (listitem, "");
			} else {
				for(int i=0; i<lHeader.size(); i++) {
