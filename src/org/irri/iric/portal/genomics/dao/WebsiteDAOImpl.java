	...
	private String writeGenelistFile(Collection genes, String site, Map params) {
		...
		try {
			...
			while(itGenes.hasNext()) {
				...
				else bw.append(gene.toString());
				if(itGenes.hasNext()) bw.append("\n");
			}
			bw.flush();
			bw.close();
	...
	public List<String> getURL1(WebsiteQuery query) {
		...
		while(itSites.hasNext()) {
			...
			try {
				...
				AppContext.debug("Executing " + "java  -jar   \"" + AppContext.getWebclientPath() + "\" \"" + infile + "\"");
		...
		while(itSites.hasNext()) {
			...
			try {
				...
				AppContext.debug("java  -jar   \"" + AppContext.getWebclientPath() + "\" \"" + infile + "\"");
		            ...
		            //Process proc = rt.exec("java  -jar   \"" + AppContext.getWebclientPath() + "\" \"" + infile + "\"");
