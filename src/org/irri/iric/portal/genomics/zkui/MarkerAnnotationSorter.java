...
public class MarkerAnnotationSorter implements Comparator {
	...
	public int compare(Object o1, Object o2) {
		...
		if(o1 instanceof MarkerAnnotation) {
			...
			if(columnname!=null) {	
				String s1=""; String s2="";
				if(l1.getAnnotation(columnname)!=null && !l1.getAnnotation(columnname).isEmpty()) s1= ((Locus) l1.getAnnotation(columnname).iterator().next()).getUniquename();
				if(l2.getAnnotation(columnname)!=null && !l2.getAnnotation(columnname).isEmpty()) s2= ((Locus) l2.getAnnotation(columnname).iterator().next()).getUniquename();
				v=s1.compareTo(s2);
			} else if(index==0) { // || index==1) {
					v=l1.getContig().compareTo(l2.getContig());
					if(v==0) v=l1.getPosition().compareTo(l2.getPosition());
			}	
			else if(index==1) { // || index==1) {
				if(l1.getMinusLogPvalue()==null || l2.getMinusLogPvalue()==null) return 0;
