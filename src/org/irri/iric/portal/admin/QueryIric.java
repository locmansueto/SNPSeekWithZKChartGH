...
import org.irri.iric.portal.chado.dao.VSnpAllvarsMinDAOImpl;
import org.irri.iric.portal.chado.dao.VSnpAllvarsPosDAO;
import org.irri.iric.portal.chado.dao.VSnpAllvarsPosDAOImpl;
...
public class QueryIric {
	...
	private static final String CONFIG_PATH = "classpath*:applicationContext-business.xml";
	...
	public QueryIric() {
		...
		allvarsdao = (VSnpAllvarsMinDAO)AppContext.checkBean(allvarsdao, "VSnpAllvarsMinDAO");
		allvarsposdao = (VSnpAllvarsPosDAO)AppContext.checkBean(allvarsposdao, "SnpsAllvarsPosDAO");
	...
	public void createSNPFileAllvars() {
		...
		try {
			...
			for(int iChr=12; iChr>0; iChr-- ) {
			  ...
			  setsortedpos.addAll(allvarsposdao.findVSnpAllvarsPosByChrPosBetween(chr, start, end)); // .findVSnpAllvarsPosByChr(chr));
	          ...
	          //File file = new File("E:/My Document/Transfer/iricquery_out/allvarsnp-chr" + chr + ".txt");
			  File file = new File(AppContext.getTempDir() + "allvarsnp-chr" + chr + ".txt");
			  
			  System.out.println(nSnps + " total snps for chr " + chr + ", writing to file:" + file.getAbsolutePath());
	          ...
	          AppContext.startTimer();
				...
				//for(int iVar=1; iVar<=3000; iVar++) {
					...
					setsortedvars.addAll( allvarsdao.findVSnpAllvarsByChrPosBetween(chr, start, end )); // .findVSnpAllvarsMinByChr( chr ) );
					
					//if(setsortedvars.size()!=nSnps) {
					//	 System.out.println( "number of snps did not match:" + nSnps + " refpos, " + setsortedvars.size() + " varid=" +  iVar );
						//throw new RuntimeException( "number of snps did not match:" + nSnps + " refpos, " + setsortedvars.size() + " varid=" +  iVar);
					//	 continue;
					//}
					...
					while(itSet.hasNext()) {
						...
						while( snpposlist.get(snpnum).getPos().longValue()<snpvar.getPos().longValue()  ) {
							buff.append( "0\t");
							snpnum++;
						}
...
public void createSNPFile2() {
			...
			for(int iChr=12; iChr>0; iChr-- ) {
	          ...
	          AppContext.startTimer();
				...
				for(int iVar=1; iVar<=3000; iVar++) {
					...
					Set setsortedvars = new TreeSet(new SnpAllvarsVarPosSorter());
					setsortedvars.addAll( allvarsdao.findVSnpAllvarsMinByChrVar( chr , BigDecimal.valueOf(iVar)) );
	...
	class SnpAllvarsVarPosSorter implements Comparator {
		...
		public int compare(Object o1, Object o2) {
			// TODO Auto-generated method stub
			int compVar = ((SnpsAllvars)o1).getVar().compareTo(  ((SnpsAllvars)o2).getVar() );
			if(compVar==0)
