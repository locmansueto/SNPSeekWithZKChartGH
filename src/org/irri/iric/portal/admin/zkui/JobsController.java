...
public class JobsController   extends SelectorComposer<Component> {
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		if(jobs==null) {
			...
			/*
			String msg = jobsfacade.getJobStatus(jobs[0]);
			try {
		...
		if(user!=null && user[0].equals("bossloc")) {
			...
			} else jobsfacade.setLogDest(false,false);
			...
			if(context!=null) {
				...
				else old = AppContext.setHostname(context[0]);
			...
			if(context!=null) {
				...
				else old = AppContext.setHostDirectory(context[0]);
			...
			if(bucket!=null) {
				...
				if(bucket[0].toLowerCase().equals("null"))
					old = AppContext.setHostDirectory(null);
				else old = AppContext.setHostDirectory(bucket[0]);
...
/*
			...
			try {
				...
				try {
					...
					if(msg.equals(JobsFacade.JOBSTATUS_DONE)) {
						/*
						if(jobsfacade.useS3())
							buttonDownloadResult.setHref("https://s3-ap-southeast-1.amazonaws.com/snp-seek-jobs/" + jobid + ".zip");
						else 
							buttonDownloadResult.setHref(null);
						*/
						buttonDownloadResult.setVisible(true);
					...
					if(msg.equals(JobsFacade.JOBSTATUS_ERROR)) {
						/*
						if(jobsfacade.useS3())
							buttonDownloadMessage.setHref("https://s3-ap-southeast-1.amazonaws.com/snp-seek-jobs/" + jobid + ".error");
						else 
							buttonDownloadMessage.setHref(null);
							*/
	...
	private void updateJoblist() {
		List listjobs=jobsfacade.getAllJobs(checkboxShowRunning.isChecked(),checkboxShowDone.isChecked(), checkboxShowDownloaded.isChecked());
	...
	public void onselectJob() {
		...
		if(job.getStatus()==null) buttonCancelJob.setDisabled(true);
		else if(job.getStatus().equals(JobsFacade.JOBSTATUS_DONE) || job.getStatus().equals(JobsFacade.JOBSTATUS_DOWNLOADED)) 
			buttonCancelJob.setDisabled(true);
		else buttonCancelJob.setDisabled(false);
	}
	...
	public void onclickDownloadresult() throws Exception {
		...
		AppContext.debug("snpallvars download complete!"+ jobid +  " Downloaded to:"  +  zksession.getRemoteHost() + "  "  +  zksession.getRemoteAddr()  );
