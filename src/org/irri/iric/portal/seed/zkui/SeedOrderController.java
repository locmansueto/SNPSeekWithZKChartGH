...
*/
	...
	public void doAfterCompose(Window comp) throws Exception {
		...
		// initialization 
	 	...
	 	if(listboxVarlists.getRows()>0 ) listboxVarlists.setSelectedIndex(0);
	...
	public void getMyChildren(Component c, int level) {
		String label="";
		if(c instanceof Label) label=((Label)c).getValue();
		
		//System.out.println(level +"  " + c.getId() + "  " + c.getClass() + " " + label +  "  atts:" + c.getAttributes().keySet());
	...
	public void onclickacceptsmta() {
		if(checkboxAcceptSMTA.isChecked()) {
	...
	public void onclickshrink() {
		if(radioShrink.isSelected()) {
			rowAuthorized.setVisible(false);
		} else rowAuthorized.setVisible(true);
	}
	...
	public void onclicksigned() {
		...
		} else rowAuthorized.setVisible(false);
	}

	
	@Listen("onClick =#buttonAddAccession")
	public void onclickAccession() {
		if(listboxAccession.getSelectedItem().getLabel().isEmpty()) return;
		try {
		if(listboxCategory.getSelectedItem().getValue().equals("pub") && listboxCountry.getSelectedItem().getLabel().isEmpty()) {
			Messagebox.show("Select your country first");
			return;
		}
		
		//Set vars=variety.getGermplasmByAccession(listboxAccession.getSelectedItem().getLabel(), new HashSet(variety.getDatasets()));
		InlineEditingViewModel vm= (InlineEditingViewModel)BinderUtil.getBinder(divBindgrid).getViewModel(); 
		List l=new ArrayList();

		l.add(variety.getGermplasmByAccession(listboxAccession.getSelectedItem().getLabel(), new HashSet(variety.getDatasets())));
		
		 //setCategoryCountry(String usercat, String country) 
		 
		OrderData o= vm.getData();
		o.setCategoryCountry((String)listboxCategory.getSelectedItem().getValue(), listboxCountry.getSelectedItem().getLabel());
	...
	public void onclickVarname() {
		if(listboxVarname.getSelectedItem().getLabel().isEmpty()) return;
		...
		List l=variety.getGermplasmByName(listboxVarname.getSelectedItem().getLabel(), new HashSet(variety.getDatasets()));
		InlineEditingViewModel vm= (InlineEditingViewModel)BinderUtil.getBinder(divBindgrid).getViewModel(); 
	...
	public void onclickVarlist() {
		...
		for(Variety v:sVars) {
			if(v.getAccession()!=null && v.getAccession().startsWith("IRGC")) l.add(v);
		}
		...
		lVars.addAll(workspace.getVarieties(listname));
		SimpleListModel m=new SimpleListModel(lVars); 
		m.setMultiple(true);
	...
	public void selectcountry() {
		...
		if(usercat.equals("pub")) {
			...
			if(ccat.equals("low")) textboxCountryCategory.setValue("Public sector in low income country");
	...
	private boolean validateFields() {
		...
		if(listboxCountry.getSelectedItem().getLabel().isEmpty() && listboxCategory.getSelectedItem().getValue().equals("pub")) {
	...
	public void onclickOrder() {
		...
		try {
			...
			o.setRequestorInfo(textboxName.getValue() , textboxInstitution.getValue(),
					textboxAddress1.getValue() , textboxAddress2.getValue(), listboxCountry.getSelectedItem().getLabel(), (String)listboxCategory.getSelectedItem().getValue(),
				   textboxEmailReq.getValue(), textboxPhoneReq.getValue(), textboxPostalCode.getValue());
			//		textboxAutname.getValue(),textboxPosition.getValue(), "","");
			
			o.setAuthorizedInfo(  (checkboxAcceptSMTA.isChecked()?(radioShrink.isSelected()?"shrink-wrap":"signed"):"not-smta") ,textboxAutname.getValue(),textboxPosition.getValue(), textboxAutaddress.getValue(), textboxPhoneAuth.getValue(), textboxEmailAuth.getValue());
			orderservice=(SeedOrderService)AppContext.checkBean(orderservice, "SeedOrderService");
			...
			if(pdf!=null) {
				...
				/*iframePDF.setSrc(pdf);
				divSMTA.setVisible(true);
				*/
	...
	public void onclickSendOrder() {
		...
		//AppContext.getHostname()+"/temp/"+orderidonly+"/smta.pdf";
			...
			if(listboxPdfMethod.getSelectedItem().getLabel().equals("docx"))
				success=orderservice.sendEmail(o.getEmailReq(), AppContext.getTempDir() +  "smta" + o.getOrderid() +".docx", "smta_" + o.getOrderid().replace("#","")+".docx" , "SMTA for #" +  o.getOrderid() );
			else success=orderservice.sendEmail(o.getEmailReq(), AppContext.getTempDir() +  o.getOrderid() +"/smta.pdf", "smta_" + o.getOrderid().replace("#","")+".pdf" , "SMTA for #" +  o.getOrderid() );
			if(success) Messagebox.show("Order sent.");
			else  Messagebox.show("Order failed to sent.");
		}catch(Exception ex) {
			ex.printStackTrace();
	...
	public void onchangeallgrams() {
		...
		setGram(spinnerAllGrams.intValue(), lseeds);
		ListModel m=gridOrders.getModel();
		m.notifyAll();
		//ListDataListiner dl=gridOrders.getl
		 * 
		 */
	}
	
/*
	private void setGram(Integer gram, List<Seed> allSeeds) {
		for(Seed s:allSeeds) {
			s.setGram(gram); 
		}
		//gridOrders.getm
	...
	private Double getTotalGram(List<Seed> allSeeds) {
		double total=0;
		for(Seed s:allSeeds) {
			total+=s.getGram();
		}
	...
	private Double getTotalPrice(List<Seed> allSeeds) {
		double total=0;
		for(Seed s:allSeeds) {
			total+=s.getPrice();
		}
