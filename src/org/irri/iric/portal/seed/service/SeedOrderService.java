...
public class SeedOrderService {
	...
	private boolean generateHtml4Doc4j(OrderData order, String orderid, int page) throws Exception {
		try {
			...
			copy(new File(AppContext.getFlatfilesDir()+ "a_bc083e_template"), new File(AppContext.getTempDir() + orderid));
			AppContext.debug("copy " + AppContext.getFlatfilesDir()+ "a_bc083e_template  to  " + AppContext.getTempDir() + orderid);
			
			String govbody="the Food and Agriculture Organization of the United Nations";
			BufferedWriter bw=new BufferedWriter(new FileWriter(AppContext.getTempDir() + orderid+"/word/document.xml"));
			BufferedReader br=new BufferedReader(new FileReader(AppContext.getFlatfilesDir()+"document1.xml"));
			String l=null;		
			while((l=br.readLine())!=null){
				
				  bw.append(l.replace("RECEPIENTNAMEADDRESS",order.getRecepientNameAddress()).replace("PROVIDERNAMEADDRESS",order.getProviderNameAddress())
						  .replace("PROVIDERAUTHNAME",order.getProviderAuthname()).replace("RECEPIENTAUTHNAME", order.getRecepientAuthname()) 
						  .replace("GOVERNINGBODY",govbody).replace("MATERIALURL","")
						  );
			}
			br.close();
			...
			while((l=br.readLine())!=null){
				  bw.append(l.replace("PROVIDERAUTHNAME",order.getProviderAuthname()).replace("RECEPIENTAUTHNAME", order.getRecepientAuthname()) 
						  .replace("PRICEESTIMATE",AppContext.decf.format(order.getTotalPrice())).replace("TOTALWEIGHT", order.getTotalGram().toString())
			...
			for(int i=0; i<5; i++) bw.append(blank);
			...
			//String blank="<w:szCs w:val=\"20\"/></w:rPr></w:pPr></w:p><w:p w:rsidR=\"00582E20\" w:rsidRDefault=\"00582E20\"><w:pPr><w:spacing w:line=\"200\" w:lineRule=\"exact\"/><w:rPr><w:sz w:val=\"20\"/>";
			bw.close();
			AppContext.debug("created " +  AppContext.getTempDir() + orderid+"/document.xml");
			
			//Files.copy(new File(AppContext.getFlatfilesDir()+ "a_bc083e_template.zip"), new File(AppContext.getTempDir() +orderid + ".zip") );
			//AppContext.debug("created " +  AppContext.getTempDir() +orderid + ".zip from template" );
			
		
			
			//Files.copy(new File(AppContext.getTempDir() + orderid+"/document.xml") , new File(AppContext.getTempDir() + orderid +"/word/document.xml"));
			//AppContext.debug("copied to " +  AppContext.getTempDir() + orderid +"/word/document.xml");
			
			CreateZipMultipleFiles.zipDir(AppContext.getTempDir() +orderid + ".zip", AppContext.getTempDir() + orderid );
			AppContext.debug("created " +  AppContext.getTempDir() +orderid + ".zip");
			new File(AppContext.getTempDir() +orderid + ".zip").renameTo(new File(AppContext.getTempDir() + "smta" + orderid + ".docx"));
			AppContext.debug("created " +  AppContext.getTempDir() + "smta" + orderid + ".docx");
			

			/*
			CreateZipMultipleFiles.addFilesToExistingZip(new File(AppContext.getTempDir() +orderid + ".zip"), new File(AppContext.getTempDir() + orderid+"/document.xml"));
			AppContext.debug("added " + AppContext.getTempDir() + orderid+"/document.xml" +" to " + AppContext.getTempDir() +orderid + ".zip");
			new File(AppContext.getTempDir() +orderid + ".zip").renameTo(new File(AppContext.getTempDir() + "smta" + orderid + ".docx"));
			AppContext.debug("created " +  AppContext.getTempDir() + "smta" + orderid + ".docx");
			*/
			return true;
	...
	private boolean generateHtml4Html2PdfPhantomJS(OrderData order, String orderid, int page) throws Exception {
		try {
		if(page==2) {
			BufferedWriter bw=new BufferedWriter(new FileWriter(AppContext.getTempDir() + orderid+"/"+page+".html"));
			BufferedReader br=new BufferedReader(new FileReader(AppContext.getFlatfilesDir()+"a_bc083e/"+page+".htm"));
			String l=null;
			...
			*/
			...
			while((l=br.readLine())!=null){
			  bw.append(l.replace("RECEPIENT_NAMEADDRESS1",order.getRecepientPage2_1()).replace("RECEPIENT_ADDRESS2",order.getRecepientPage2_2()));
			}
			bw.flush();
			bw.close();
			br.close();
				...
				while((l=br.readLine())!=null){
				  bw.append(l.replace("RECEPIENT_AUTHORIZED",order.getpRecepientPage8_2()).replace("PROVIDER_AUTHORIZED",order.getProviderPage8_1()) );
				}
				bw.flush();
				bw.close();
				br.close();
			...
			bw.append("<table width=\"80%\"><tr><th>Accession</th><th>Designation</th><th>US$/10 gram</th><th>Quantity (g)</th><th>Price US$</th></tr>\n");
			for(String acc:m.keySet()) {
				Seed s=m.get(acc);
				bw.append("<tr><td>" + s.getAccession() + "</td><td>" +  s.getVarname() + "</td><td>" + s.getPricePerGram() + "</td><td>" + s.getGram() + "</td><td>" + s.getPrice() + "</td></tr>\n");
			}
			bw.append("</table>");
			...
			bw.append("<p class=MsoNormal align=right style='text-align:right'><span style='font-size:12.0pt'>9</span></p>");
	...
	public boolean sendEmail(String to, String smtppath, String smtfilename, String subject) {
		//On clicking the order button you'd generate an order ID, which you'd send to us (Marionette "Monette" Alana) and to the user, to be quoted in subsequent correspondence.
		//The email to the user user should also alert them to provide any necessary instructions and documents, e.g. shipping instructions (port of entry, shipping address, consignee, seed treatment),
		//import permit, ...
		try {
			emailservice.sendMessageWithAttachment(to, subject, "Thank you for ordering seeds from IRRI. Please sign the attached Standard Material Transfer Agreement and send it to Ms. Marionette Alana (m.alana@irri.org).\n" +
	...
	//buff.append("Your seed order #" + ordercode + " please confirm at " + " using verification code " + verifycode);
	buff.append("<h2>Seed Ordering to IRRI International Rice Genebank Collection (IRGC) via SNP-Seek</h2><br><br>");
	buff.append("Your seed order has been sent to Ms. Marionette Alana (m.alana@irri.org) of IRRI Rice Genebank Collection with the information you provided.");
	buff.append("You may send additional necessary instructions and documents, e.g. shipping instructions (port of entry, shipping address, consignee, seed treatment), import permit.");

	buff.append("<br><br><b>Seed Order #" + ordercode +"</b>\n");
	buff.append("<br><br><b>Order date:</b>" +  AppContext.datef.format( new Date()) +"</b>\n");
	buff.append("<h3>Requestor</h3><b>Name:</b>" + order.getName() +"<br><b>Institution:</b>"+ order.getInstitution() +
			"<br><b>Address:</b>" + order.getAddress1());
	if(order.getAddress2()!=null && !order.getAddress2().isEmpty()) buff.append("<br>" + order.getAddress2());
	...
	if(order.getCategory().equals("pub")) buff.append("Public sector");
	else if(order.getCategory().equals("priv")) buff.append("Private sector");
	else if(order.getCategory().equals("irri")) buff.append("within IRRI");
	...
	if(order.getCategory().equals("pub")) {
		...
		if(ccat.equals("low")) buff.append("Public sector in low income country");
		else if(ccat.equals("lowmid")) buff.append("Public sector in lower middle income country");
		else if(ccat.equals("upmid")) buff.append("Public sector in upper middle income country");
		else if(ccat.equals("high")) buff.append("Public sector in high income country");
	}
	...
	if(order.getAcceptanceType().equals("not-smta")) buff.append("The requestor did not accept the terms and conditions of the SMTA");
	else if(order.getAcceptanceType().equals("signed")) buff.append("Requestor gives advance notification that they have read the terms and conditions of the SMTA/MTA and that they intend to accept these terms and conditions on receipt of the seed with <b>signed SMTA/MTA</b>.");
	else if(order.getAcceptanceType().equals("shrink-wrap")) buff.append("Requestor gives advance notification that they have read the terms and conditions of the SMTA/MTA and that they intend to accept these terms and conditions on receipt of the seed with <b>shrinked-wrap SMTA/MTA</b>.");
...
//				new SeedOrderIRGC( order.getName() ,order.getInstitution(), order.getAddress1(), 
//						order.getAddress2() ,	order.getCountry() ,	order.getEmailReq() , order.getPhoneReq() ,order.getCategory() ,
//						getCountryCategory(order.getCountry()), order.getAcceptanceType() ,order.getAutname() ,order.getPosition(),
//						 order.getEmailAuth() ,order.getPhoneAuth() ,order.getPostalCode() ,calNow , STATUS_SUBMITTED, 
//						 order.getTotalPrice() ,verifycode ,ordercode);
//		seedorderservice.saveSeedOrder(orderirgc);	
//		SeedOrderIRGC orderirgcloaded= seedorderservice.findSeedOrderByDateOrderCode(calNow, ordercode);
...
//		// generate email
//		StringBuffer buff=new StringBuffer();
//		buff.append("Your seed order code " + orderid + " please confirm at " + " using verification code " + verifycode);
//		for(Seed seed:order.getAllSeeds()) {
//			buff.append( seed.getAccession() +"\t" + seed.getVarname() + "\t" + seed.getGram() +" g\tUS$ " + seed.getPrice() +"\n" );
//		}
//		
//		List lto=new ArrayList();
//		lto.add(order.getEmailReq());
//		if(order.getEmailAuth()!=null && !order.getEmailAuth().isEmpty()) lto.add(order.getEmailAuth());
//		List lcc=new ArrayList();
//		lcc.add("l.mansueto@irri.org");
...
//					
//					
//					//AppContext.debug("http://localhost:8080/temp/a_bc083e/"+i+".html converting to " +   AppContext.getTempDir()+"test_pdf/"+i+".pdf");
//					//new C07E04_CreateFromURL().createPdf(new URL("http://localhost:8080/temp/a_bc083e/"+i+".html"), AppContext.getTempDir()+"test_pdf/"+i+".pdf");
//					if(i==2 || i== 8 || i== 12 || i==9) {
//					if(generateHtml4Html2PdfPhantomJS( order, orderidonly, i ))
//					h2p.createPdf( AppContext.getHostname()+"/temp/"+orderidonly+"/"+i+".html",  orderid+"/"+i+".pdf" );
//					}
//					else {
//						//copy pdf
//						//new File( AppContext.getTempDir() + orderid+"/"+i+".pdf").createNewFile();
//						Files.copy(new File(AppContext.getFlatfilesDir()+"a_bc083e/a-bc083e."+i+".pdf"),new File(  orderid+"/"+i+".pdf"));
//					}
...
//			
//			/*
//			DocxToPDFConverter c1=new DocxToPDFConverter(new FileInputStream(new File(AppContext.getTempDir() + "smta" + orderidonly + ".docx")), 
//					new FileOutputStream(new File(AppContext.getTempDir() + "smta" + orderidonly + ".pdf")), true, true);
//			c1.convert();
//			*/
//			Docx2PdfConverter c=new Docx2PdfConverter();
//			//c.convert2Pdf(AppContext.getFlatfilesDir()+"a_bc083e.docx", AppContext.getTempDir() + "smta" + orderidonly + ".pdf",pdfmethod);
//			c.convert2Pdf(AppContext.getTempDir() + "smta" + orderidonly + ".docx", AppContext.getTempDir() + "smta" + orderidonly + ".pdf",pdfmethod);
//			
//			/*
//			c.convert2Pdf(AppContext.getFlatfilesDir()+"a_bc083e_template.docx", AppContext.getTempDir() + "smta_template.pdf",pdfmethod);
//			c.convert2Pdf(AppContext.getFlatfilesDir()+"a_bc083e_template.docx", AppContext.getTempDir() + "smta" + orderidonly + ".pdf",pdfmethod);
//			*/
