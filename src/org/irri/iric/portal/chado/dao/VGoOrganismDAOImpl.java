...
public class VGoOrganismDAOImpl extends AbstractJpaDao<VGoOrganism> implements
	...
	@SuppressWarnings("unchecked")
	@Transactional
	public Set<VGoOrganism> findVGoOrganismByCvCommonName(String cv, String commonName, int startResult, int maxRows) throws DataAccessException {
		Query query = createNamedQuery("findVGoOrganismByCvCommonName", startResult, maxRows, cv, commonName);
		return new LinkedHashSet<VGoOrganism>(query.getResultList());
	}
	...
	public VGoOrganism findVGoOrganismByPrimaryKey(Integer cvtermId, int startResult, int maxRows) throws DataAccessException {
		try {
			...
			return (org.irri.iric.portal.chado.domain.VGoOrganism) query.getSingleResult();
